...
function renderTeams(teams) {
  ...
  teams.forEach((t, i) => {
    ...
    a.href = t.href || "#";
    a.className =
      "group overflow-hidden rounded-2xl border bg-white shadow-sm hover:shadow-md transition reveal";
    a.innerHTML = `
...
function renderNews(news) {
  ...
  [...news].sort((a,b)=>b.date.localeCompare(a.date)).forEach((n) => {
    ...
    li.className = "rounded-2xl border border-blue-100 bg-white p-5 reveal";
    li.innerHTML = `
...
function renderSchedule(schedule) {
  ...
  wrap.innerHTML = "";
  schedule.sort((a,b)=>a.date.localeCompare(b.date)).forEach((s) => {
    ...
    const badgeCls = isHome ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700";
    const badgeTxt = isHome ? "Heim" : "Auswärts";

    const card = document.createElement("div");
    card.className = "group rounded-2xl border bg-white p-4 flex gap-4 items-start transition transform hover:-translate-y-0.5 hover:bg-blue-50 hover:border-blue-300 reveal";
    card.innerHTML = `
...
document.addEventListener("DOMContentLoaded", () => {
  const toggleButton = byId("menu-toggle");
